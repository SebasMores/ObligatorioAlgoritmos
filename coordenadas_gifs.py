...
if __name__ == "__main__":
    ...
    if start not in valid_nodes:
        print(
            "El punto de origen no esta en la red vial principal. Buscando alternativa..."
        )
        start = min(
            valid_nodes,
            key=lambda n: (
                (G.nodes[n]["x"] - start_coords[1]) ** 2
                + (G.nodes[n]["y"] - start_coords[0]) ** 2
            )
            ** 0.5,
        )
    if end not in valid_nodes:
        print(
            "El punto de destino no esta en la red vial principal. Buscando alternativa..."
        )
        end = min(
            valid_nodes,
            key=lambda n: (
                (G.nodes[n]["x"] - end_coords[1]) ** 2
                + (G.nodes[n]["y"] - end_coords[0]) ** 2
            )
            ** 0.5,
        )
